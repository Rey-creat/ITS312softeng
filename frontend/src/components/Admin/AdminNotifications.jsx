                                        ...
                                        {filteredRequests
                                            ...
                                            .map((req) => (
                                                <tr key={req.id} className="hover:bg-gray-50 transition-colors whitespace-nowrap">
                                                    <td className="px-5 py-4 text-sm font-bold text-blue-700">REQ-{req.id}</td>
                                                    <td className="px-5 py-4 text-sm text-gray-700">{formatDate(req.date_filed)}</td>
                                                    <td className={`px-5 py-4 text-sm font-medium ${new Date(req.date_needed) < new Date() ? "text-red-600" : "text-gray-700"}`}>{formatDate(req.date_needed)}</td>
                                                    <td className="px-5 py-4 text-sm text-gray-700 font-medium">{req.type_of_concern}</td>
                                                    <td className="px-5 py-4 text-sm text-gray-700">{req.description}</td>
                                                    <td className="px-5 py-4 text-sm font-medium text-gray-900">{req.requested_by}</td>
                                                    <td className="px-6 py-4 font-medium text-green-700">{req.noted_by || "—"}</td>
                                                    ...
                                                    <td className="px-6 py-4">
                                                        {req.status === 'Done' ? (
                                                            <span className="inline-block px-3 py-1.5 rounded-full text-xs font-semibold bg-green-200 text-green-800 border border-green-300">Completed</span>
                                                        ) : req.assigned_to ? (
                                                            <span className="inline-flex items-center gap-2">
                                                                <span className="font-medium text-green-700">{req.assigned_to}</span>
                                                                <span className="text-xs text-gray-500">Assigned</span>
                                                                <button
                                                                    onClick={() => {
                                                                        if (!isReassignDisabled(req)) openAssignModal(req.id);
                                                                    }}
                                                                    className={`px-3 py-1.5 text-xs font-medium text-blue-600 bg-blue-50 hover:bg-blue-100 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed ${isReassignDisabled(req) ? 'opacity-50 cursor-not-allowed' : ''}`}
