	...
	const handleAssign = async (requestId, personnelId) => {
		...
		try {
			...
			await axios.post(
				...
				{ personnelId: parseInt(personnelId) }, // Ensure it's a number if backend expects int
				{ headers: { Authorization: `Bearer ${token}` } }
			);
			setRequests((prev) =>
				prev.map((r) =>
					r.id === requestId ? { ...r, assigned_to: personnelId } : r
				)
			);
					...
					<table className="w-full text-sm mb-8">
						<thead className="bg-gray-200">
							<tr>
								...
								{/* <th className="px-3 py-2 text-left">PPGS Approved</th> */}
								{/* <th className="px-3 py-2 text-left">President Approved</th> */}
						...
						<tbody className="divide-y divide-gray-200">
							{requests.map((req) => (
								<tr key={req.id}>
									...
									{/* <td className="px-3 py-2">{req.ppgs_approved_by || "—"}</td> */}
									{/* <td className="px-3 py-2">{req.approved_by || "—"}</td> */}
